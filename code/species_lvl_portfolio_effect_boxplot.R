...
total_harvest_sp <- simulated_harvest_df %>%
  filter(!is.na(harvest_amount_kg)) %>%
  group_by(site, date, species) %>%
  summarise_at(vars(harvest_amount_kg), list(harvest_total = sum)) 


total_harvest_sp_cv <- total_harvest_sp %>%
  group_by(site, species) %>%
  mutate(mean_h = mean(harvest_total),
         sd_h = sd(harvest_total),
         cv_h = sd_h/mean_h) %>%
  ungroup() %>%
  select(site, species, mean_h:cv_h) %>%
  distinct()

seasonal_mean_df_h_cv <- total_harvest_sp_cv %>%
  ungroup() %>%
  group_by(site) %>%
  mutate(seasonal_mean_sp_cv = mean(cv_h)) %>%
  select(site, seasonal_mean_sp_cv) %>% 
  distinct()
...
seasonal_harvest_cv <- total_simulated_harvest_cv %>%
  ...
  rename(mean_sp_cv = "seasonal_mean_sp_cv", Community = "site") %>%
...
df_h <- df_temp_avg %>%
  ...
  group_by(Site_Year_Code, Lowest_Common_Taxon_Name) %>%
...
df_h_cv <- df_h %>%
  ungroup() %>%
  group_by(Community, Lowest_Common_Taxon_Name) %>%
...
mean_df_h_cv <- df_h_cv %>%
  ...
  filter(!is.na(cv_h)) %>%
  group_by(Community) %>%
  mutate(mean_sp_cv = mean(cv_h)) %>%
  select(Community, mean_sp_cv) %>% 
  distinct()
...
harvest_cv_all <- rbind(decadal_harvest_cv, seasonal_harvest_cv) %>%
  pivot_longer(cols = c("cv_th", "mean_sp_cv"), names_to = "cv_type", values_to = "CV") %>%
  mutate(`CV Type` = case_when(
    startsWith(cv_type, "cv") ~ "Total Harvest CV",
    startsWith(cv_type, "mean") ~ "Mean Species CV",
  ))
